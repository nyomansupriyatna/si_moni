                ...
                <td colspan="{{ count($headers)+1 }}">
                    <div class="block mb-1 bg-indigo-700 sm:flex">
                        ...
                        <div class="flex w-full px-3 py-2" >
                            <h2 class="justify-start h-full font-bold text-white items-center">Mapping Regu List</h2>
                        </div>
                        <div class="flex w-full p-2 lg:justify-end lg:w-7/12">
                            <div class="relative">
                                <div class="absolute flex items-center h-full items-center left-1">
                                    <x-icon-search width=20 height=20 class="text-gray-400"/>
                                </div>
                                <div wire:click="clearSearch" class="absolute flex items-center h-full cursor-pointer items-center right-1">
            ...
            <thead class="px-3 font-bold text-indigo-700 bg-indigo-200">
                @foreach($headers as $key => $value)
                    ...
                    <th wire:click="sort('{{ $key }}')" class="py-2 border border-gray-400 cursor-pointer w-min hover:bg-indigo-300 hover:text-indigo-900">
                        <div class="flex items-center justify-center gap-2 items-center">
                            @if($sortColumn == $key)
                ...
                @endforeach
                    <td class="w-auto py-2 border border-gray-400">
                        ...
                        @if($isOpen)
                            @include('livewire.mapping-regu.create')
                        @endif
                    </td>
            </thead>
            <tbody>
