                ...
                <td colspan="{{ count($headers)+1 }}">
                    <div class="block mb-1 bg-indigo-700 sm:flex">
                        ...
                        <div class="flex w-full px-3 py-2" >
                            <h2 class="justify-start h-full font-bold text-white users-center">User List</h2>
                        </div>
                        <div class="flex w-full p-2 lg:justify-end lg:w-7/12">
                            <div class="relative">
                                <div class="absolute flex items-center h-full users-center left-1">
                                    <x-icon-search width=20 height=20 class="text-gray-400"/>
                                </div>
                                <div wire:click="clearSearch" class="absolute flex items-center h-full cursor-pointer users-center right-1">
            ...
            <thead class="px-3 font-bold text-indigo-700 bg-indigo-200">
                @foreach($headers as $key => $value)
                    ...
                    <th wire:click="sort('{{ $key }}')" class="py-2 border border-gray-400 cursor-pointer w-min hover:bg-indigo-300 hover:text-indigo-900">
                        <div class="flex items-center justify-center gap-2 users-center">
                            @if($sortColumn == $key)
                        ...
                        <tr class="hover:bg-gray-200">
                            ...
                            <td class="border border-gray-300 ">
                                <div class="flex items-center justify-around gap-2 mx-1">
                                    <x-btn-edit  title="" wire:click="edit({{$user->id}})" />
                                    <x-btn-delete  title="" wire:click="showDelete({{$user->id}})" />
                                </div>
                            </td>
                        </tr>
                    @endforeach
